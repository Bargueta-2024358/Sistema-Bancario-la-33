...
export const createProduct = async (req, res) => {
  try {
    ...
    const { currencyCode: _cc, currency: _c, ...rest } = req.body;
...
export const updateProduct = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, interestRate, currencyCode } = req.body;
    ...
    if (currencyCode) {
      const currencyDoc = await Currency.findOne({
        code: String(currencyCode).trim().toUpperCase(),
      });
      if (!currencyDoc) {
        return res.status(400).json({
          success: false,
      ...
      updateData.currency = currencyDoc._id;
    }

    const product = await updateProductRecord({ id, updateData });
